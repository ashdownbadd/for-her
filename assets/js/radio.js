...
export function createRadio(position, rotationY = 0) {
    ...
    const bodyMat = new THREE.MeshPhysicalMaterial({ color: woodColor, roughness: 0.3, clearcoat: 0.5 });
    const radioBody = new THREE.Mesh(new THREE.ExtrudeGeometry(shape, { depth: depth, bevelEnabled: false }), bodyMat);
    ...
    const upperFace = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: creamColor })
    );
    ...
    const lowerFace = new THREE.Mesh(
        new THREE.PlaneGeometry(width - 0.08, height / 2 - 0.04),
        new THREE.MeshStandardMaterial({ color: darkAccent })
    );
    ...
    const dotMat = new THREE.MeshStandardMaterial({ color: 0x000000, transparent: true, opacity: 0.2 });
    for (let i = 0; i < 12; i++) {
        for (let j = 0; j < 6; j++) {
    ...
    const winMat = new THREE.MeshStandardMaterial({ color: 0x1a1a1a, emissive: amberGlow, emissiveIntensity: 0.4 });
    ...
    const knobMat = new THREE.MeshStandardMaterial({ color: creamColor });
    ...
    const footPos = [[-0.2, -0.015, 0.08], [0.2, -0.015, 0.08], [-0.2, -0.015, -0.08], [0.2, -0.015, -0.08]];
    footPos.forEach(p => {
    ...
    radioGroup.traverse(c => { if (c.isMesh) { c.castShadow = true; c.receiveShadow = true; } });
